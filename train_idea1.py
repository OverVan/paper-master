...
if __name__ == "__main__":
    ...
    config_path = "config/train_idea1_{}-shot_mini_resnet12.yaml".format(shot)
    with open(config_path, "r", encoding="UTF-8") as file:
        config = yaml.load(file, yaml.FullLoader)
        
    load_dir = os.path.join("save", "{}_{}".format(config["train_dataset"]["name"], config["model"]["args"]["encoder"]["name"]))
    save_dir = os.path.join("idea1_checkpoints", "{}-shot_{}_{}".format(shot, config["train_dataset"]["name"], config["model"]["args"]["encoder"]["name"]))
    ...
    train_dataset = SimpleDataset(**config["train_dataset"]["args"], phase="train_idea1")
    ...
    val_dataset = SimpleDataset(**config["val_dataset"]["args"], phase="train_idea1")
    ...
    if os.path.exists(last_path):
        ...
        log("continue to train from epoch {}".format(start_epoch + 1), "train_idea1")
        ...
        log("train {}-shot {} {}".format(shot, config["train_dataset"]["name"], config["model"]["args"]["encoder"]["name"]), "train_idea1")
        start_epoch = 0
    ...
    for epoch in range(start_epoch, max_epoch):
        ...
        log("epoch {}:\n\ttrain loss: {:.4f}\ttrain acc: {:.2%}\n\tval loss: {:.4f}\tval acc: {:.2%}".format(epoch_id, train_loss, train_acc, val_loss, val_acc), "train_idea1")
        
        if val_acc >= max_acc:
            max_acc = val_acc
            best_epoch = epoch_id
            torch.save(model.state_dict(), max_path)
        log("\tbest epoch: {}\tmax acc: {:.2%}".format(best_epoch, max_acc), "train_idea1")
        checkpoint = {
            "last_epoch": epoch_id,
