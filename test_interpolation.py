...
if __name__ == "__main__":
    ...
    config_path = "config/test_interpolation_mini_resnet12.yaml"
    with open(config_path, "r", encoding="UTF-8") as file:
        config = yaml.load(file, yaml.FullLoader)
        
    save_dir = os.path.join("interpolation_checkpoints", "{}_{}".format(config["test_dataset"]["name"], config["model"]["args"]["encoder"]["name"]))
    ...
    log("test interpolation (1-shot) {} {} {}-way".format(config["test_dataset"]["name"], config["model"]["args"]["encoder"]["name"], config["test_ep"]["n"]), path="test_interpolation")
    ...
    for epoch in range(max_epoch):
        ...
        log("epoch {}:\n\tloss: {:.4f}\tacc: {:.2f} +- {:.2f} (%)".format(epoch_id, test_loss, np.mean(test_acc) * 100, mean_confidence_interval(test_acc) * 100), path="test_interpolation")
    
    final_loss = np.mean(final_loss)
    log("final:\n\tloss: {:.4f}\tacc: {:.2f} +- {:.2f} (%)".format(final_loss, np.mean(final_acc) * 100, mean_confidence_interval(final_acc) * 100), path="test_interpolation")