...
if __name__ == "__main__":
    ...
    config_path = "config/test_idea2_{}-shot_mini_resnet12.yaml".format(shot)
    ...
    test_dataset = SimpleDataset(**config["test_dataset"]["args"], phase="test_idea2")
    ...
    log("test {}-shot {} {}".format(shot, config["test_dataset"]["name"], config["model"]["args"]["encoder"]["name"]), path="test_idea2")
    ...
    for epoch in range(max_epoch):
        ...
        log("epoch {}:\n\tloss: {:.4f}\tacc: {:.2f} +- {:.2f} (%)".format(epoch_id, test_loss, np.mean(test_acc) * 100, mean_confidence_interval(test_acc) * 100), path="test_idea2")
    
    final_loss = np.mean(final_loss)
    log("final:\n\tloss: {:.4f}\tacc: {:.2f} +- {:.2f} (%)".format(final_loss, np.mean(final_acc) * 100, mean_confidence_interval(final_acc) * 100), path="test_idea2")