...
if __name__ == "__main__":
    ...
    config_path = "config/test_extrapolation_mini_resnet12.yaml"
    ...
    test_dataset = dataset.make(config["test_dataset"]["type"], **config["test_dataset"]["args"], phase="test_extrapolation")
    ...
    log("test extrapolation (1-shot) {} {} {}-way".format(config["test_dataset"]["name"], config["model"]["args"]["encoder"]["name"], config["test_ep"]["n"]), path="test_extrapolation")
    ...
    for epoch in range(max_epoch):
        ...
        log("epoch {}:\n\tloss: {:.4f}\tacc: {:.2f} +- {:.2f} (%)".format(epoch_id, test_loss, np.mean(test_acc) * 100, mean_confidence_interval(test_acc) * 100), path="test_extrapolation")
    
    final_loss = np.mean(final_loss)
    log("final:\n\tloss: {:.4f}\tacc: {:.2f} +- {:.2f} (%)".format(final_loss, np.mean(final_acc) * 100, mean_confidence_interval(final_acc) * 100), path="test_extrapolation")