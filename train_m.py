...
if __name__ == "__main__":
    ...
    save_dir = os.path.join("m_checkpoints", "{}-shot_{}_{}".format(shot, config["train_dataset"]["name"], config["model"]["args"]["encoder"]["name"]), "m={}".format(config["train_ep"]["q"]))
    ...
    train_dataset = SimpleDataset(**config["train_dataset"]["args"], phase="train_m")
    ...
    val_dataset = SimpleDataset(**config["val_dataset"]["args"], phase="train_m")
    ...
    if os.path.exists(last_path):
        ...
        log("continue to train from epoch {}".format(start_epoch + 1), "train_m")
        model.load_state_dict(checkpoint["model_sd"])
        max_acc = checkpoint["max_acc"]
        best_epoch = checkpoint["best_epoch"]
    else:
        log("train {}-shot {} {} m = {}".format(shot, config["train_dataset"]["name"], config["model"]["args"]["encoder"]["name"], config["train_ep"]["q"]), "train_m")
        start_epoch = 0
    ...
    for epoch in range(start_epoch, max_epoch):
        ...
        log("epoch {}:\n\ttrain loss: {:.4f}\ttrain acc: {:.2%}\n\tval loss: {:.4f}\tval acc: {:.2%}".format(epoch_id, train_loss, train_acc, val_loss, val_acc), "train_m")
        
        if val_acc >= max_acc:
            max_acc = val_acc
            best_epoch = epoch_id
            torch.save(model.state_dict(), max_path)
        log("\tbest epoch: {}\tmax acc: {:.2%}".format(best_epoch, max_acc), "train_m")
        checkpoint = {
            "last_epoch": epoch_id,
    ...
    ablation["m"].append(m)
    ablation["val_acc"].append(max_acc)
    torch.save(ablation, "m.pth")
    print("over")