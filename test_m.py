...
if __name__ == "__main__":
    ...
    save_dir = os.path.join("m_checkpoints", "{}-shot_{}_{}".format(shot, config["test_dataset"]["name"], config["model"]["args"]["encoder"]["name"]), "m={}".format(config["test_ep"]["q"]))
    max_path = os.path.join(save_dir, "max-val.pth")
        
    test_dataset = SimpleDataset(**config["test_dataset"]["args"], phase="test_m")
    ...
    log("test {}-shot {} {} m = {}".format(shot, config["test_dataset"]["name"], config["model"]["args"]["encoder"]["name"], config["test_ep"]["q"]), path="test_m")
    ...
    for epoch in range(max_epoch):
        ...
        log("epoch {}:\n\tloss: {:.4f}\tacc: {:.2f} +- {:.2f} (%)".format(epoch_id, test_loss, np.mean(test_acc) * 100, mean_confidence_interval(test_acc) * 100), path="test_m")
    
    final_loss = np.mean(final_loss)
    log("final:\n\tloss: {:.4f}\tacc: {:.2f} +- {:.2f} (%)".format(final_loss, np.mean(final_acc) * 100, mean_confidence_interval(final_acc) * 100), path="test_m")
    
    ablation = torch.load("m.pth")
    ablation["test_acc"].append(np.mean(final_acc))
