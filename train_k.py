...
if __name__ == "__main__":
    ...
    save_dir = os.path.join("k_checkpoints", "{}-shot_{}_{}".format(shot, config["train_dataset"]["name"], config["model"]["args"]["encoder"]["name"]), "k={}".format(config["model"]["args"]["k"]))
    ...
    train_dataset = SimpleDataset(**config["train_dataset"]["args"], phase="train_k")
    ...
    val_dataset = SimpleDataset(**config["val_dataset"]["args"], phase="train_k")
    ...
    if os.path.exists(last_path):
        ...
        log("continue to train from epoch {}".format(start_epoch + 1), "log/train_k.txt")
        ...
        log("train {}-shot {} {} k={}".format(shot, config["train_dataset"]["name"], config["model"]["args"]["encoder"]["name"], config["model"]["args"]["k"]), "log/train_k.txt")
        start_epoch = 0
    ...
    for epoch in range(start_epoch, max_epoch):
        ...
        log("epoch {}:\n\ttrain loss: {:.4f}\ttrain acc: {:.2%}\n\tval loss: {:.4f}\tval acc: {:.2%}".format(epoch_id, train_loss, train_acc, val_loss, val_acc), "log/train_k.txt")
        
        if val_acc >= max_acc:
            max_acc = val_acc
            best_epoch = epoch_id
            torch.save(model.state_dict(), max_path)
        log("\tbest epoch: {}\tmax acc: {:.2%}".format(best_epoch, max_acc), "log/train_k.txt")
        checkpoint = {
            "last_epoch": epoch_id,
