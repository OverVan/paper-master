...
if __name__ == "__main__":
    ...
    save_dir = os.path.join("k_checkpoints", "{}-shot_{}_{}".format(shot, config["test_dataset"]["name"], config["model"]["args"]["encoder"]["name"]), "k={}".format(config["model"]["args"]["k"]))
    max_path = os.path.join(save_dir, "max-val.pth")
        
    test_dataset = SimpleDataset(**config["test_dataset"]["args"], phase="test_k")
    ...
    log("test {}-shot {} {} k={}".format(shot, config["test_dataset"]["name"], config["model"]["args"]["encoder"]["name"], config["model"]["args"]["k"]), path="test_k")
    ...
    for epoch in range(max_epoch):
        ...
        log("epoch {}:\n\tloss: {:.4f}\tacc: {:.2f} +- {:.2f} (%)".format(epoch_id, test_loss, np.mean(test_acc) * 100, mean_confidence_interval(test_acc) * 100), path="test_k")
    
    final_loss = np.mean(final_loss)
    log("final:\n\tloss: {:.4f}\tacc: {:.2f} +- {:.2f} (%)".format(final_loss, np.mean(final_acc) * 100, mean_confidence_interval(final_acc) * 100), path="test_k")
    
    k = config["model"]["args"]["k"]
    if k != 1:
        ablation = torch.load("k.pth")
    else:
        ablation = {
            "k": [],
            "acc": []
    ...
    ablation["k"].append(k)
    ablation["acc"].append(np.mean(final_acc))
    torch.save(ablation, "k.pth")
    print("over")