...
if __name__ == "__main__":
    ...
    config_path = "config/test_{}-shot_tiered_resnet12.yaml".format(shot)
    with open(config_path, "r", encoding="UTF-8") as file:
        config = yaml.load(file, yaml.FullLoader)
        
    save_dir = os.path.join("save", "{}_{}".format(config["test_dataset"]["name"], config["model"]["args"]["encoder"]["name"]))
    load_path = os.path.join(save_dir, "max-val.pth")
        
    test_dataset = SimpleDataset(**config["test_dataset"]["args"], phase="infer")
